...
apiRoutes.post('/new_application', async (req, res) => {
    try {
        ...
        const validaDocumento = await pfSimples.findOne({ cpf: cpf });
        if (validaDocumento) return res.json({ status: false, erro: "Documento já possui requerimento (Simples) " }).status(400);

        try {
            const nova_pessoa_fisica = new pfSimples({
                cpf: cpf,
                nome: nome,
                rg: rg,
